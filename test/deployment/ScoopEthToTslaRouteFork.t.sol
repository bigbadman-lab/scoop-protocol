...
/// @notice Phase 6C.4 fork proof: post-launch ETH -> TSLA -> TOKEN in one Universal Router tx.
contract ScoopEthToTslaRouteForkTest is Test {
    ...
    address constant TSLA_USD_FEED = 0x4A1166a659A55625345e9515b32adECea5547C38;
    ...
    function test_fork_launchTimeDevBuyRequiresTsla() public {
        _configureTslaOnFork();
        ScoopFactory.LaunchParams memory params = _params("NeedTsla", "NUG", bytes32(uint256(801)));
        uint256 fee = FACTORY.LAUNCH_FEE();
        vm.startPrank(creator);
        IERC20(TSLA).approve(address(FACTORY), 0.1e18);
        vm.expectRevert();
        FACTORY.launchAndBuy{value: fee}(params, 0.1e18, 1);
        vm.stopPrank();
        assertEq(IERC20(TSLA).balanceOf(creator), 0);
    }

    function test_fork_ethOnlyBuyerSingleTxBuysTest() public {
        _configureTslaOnFork();
        _launchTestTslaWithCreatorBuy();

        assertGt(ethBuyer.balance, 0);
        assertEq(IERC20(TSLA).balanceOf(ethBuyer), 0);
        assertEq(IERC20(testToken).balanceOf(ethBuyer), 0);

        uint256 ethIn = 0.02 ether;
        uint256 tslaQuoted = _quoteWethToTsla(ethIn);
        assertGt(tslaQuoted, 0, "ETH->TSLA quote failed");
        console2.log("quoted TSLA for 0.02 ETH", tslaQuoted);

        uint256 factoryEthBefore = address(FACTORY).balance;
        uint256 factoryTslaBefore = IERC20(TSLA).balanceOf(address(FACTORY));
        ...
        IUniversalRouter(UNIVERSAL_ROUTER).execute{value: ethIn}(
            ...
            _routeInputs(ethIn, tslaQuoted),
            block.timestamp + 60
        );

        uint256 testBal = IERC20(testToken).balanceOf(ethBuyer);
        uint256 tslaBal = IERC20(TSLA).balanceOf(ethBuyer);
        uint256 wethBal = IERC20(WETH).balanceOf(ethBuyer);
        console2.log("buyer TEST", testBal);
        console2.log("buyer TSLA residual", tslaBal);
        console2.log("buyer WETH residual", wethBal);

        assertGt(testBal, 0, "buyer received no TEST");
        assertLe(tslaBal, 1e12, "unexpected TSLA residual");
        assertEq(wethBal, 0, "unexpected WETH residual");

        ScoopFactory.Launch memory rec = FACTORY.getLaunch(testToken);
        assertEq(rec.quoteAsset, TSLA);
        assertEq(PoolId.unwrap(rec.poolId), PoolId.unwrap(testPoolId));
        assertEq(address(FACTORY).balance, factoryEthBefore);
        assertEq(IERC20(TSLA).balanceOf(address(FACTORY)), factoryTslaBefore);
        assertEq(IERC20(testToken).balanceOf(address(FACTORY)), 0);
    }

    function _configureTslaOnFork() internal {
        vm.startPrank(AUTHORITY);
        if (!PRICE_ORACLE.isConfigured(TSLA)) {
            PRICE_ORACLE.configureFeed(TSLA, TSLA_USD_FEED, STOCK_MAX_AGE);
        }
        if (!QUOTE_REGISTRY.isRegistered(TSLA)) {
            QUOTE_REGISTRY.registerQuote(TSLA, ScoopQuoteRegistry.QuoteType.Stock);
        }
        vm.stopPrank();
        assertTrue(QUOTE_REGISTRY.isEnabled(TSLA));
        assertTrue(PRICE_ORACLE.isEnabled(TSLA));
    }

    function _launchTestTslaWithCreatorBuy() internal {
        ScoopFeeConfigFactoryGuard.skipUnlessFeeConfig(FACTORY);
        uint256 quoteIn = 0.1e18;
        deal(TSLA, creator, quoteIn);
        ScoopFactory.LaunchParams memory params = _params("RouteTest", "RTEST", bytes32(uint256(802)));
        uint256 fee = FACTORY.LAUNCH_FEE();
        vm.startPrank(creator);
        IERC20(TSLA).approve(address(FACTORY), quoteIn);
    ...
    function _quoteWethToTsla(uint256 amountIn) internal returns (uint256 amountOut) {
        (amountOut,,,) = IQuoterV2(V3_QUOTER)
            .quoteExactInputSingle(
                IQuoterV2.QuoteExactInputSingleParams({
                    tokenIn: WETH, tokenOut: TSLA, amountIn: amountIn, fee: V3_FEE, sqrtPriceLimitX96: 0
                })
            );
    }

    function _routeInputs(uint256 ethIn, uint256 quotedTsla) internal view returns (bytes[] memory inputs) {
        inputs = new bytes[](4);
        ...
        uint256 minTsla = quotedTsla * 99 / 100;
        bytes memory v3Path = abi.encodePacked(WETH, V3_FEE, TSLA);
        inputs[1] = abi.encode(ActionConstants.ADDRESS_THIS, ActionConstants.CONTRACT_BALANCE, minTsla, v3Path, false);

        PoolKey memory key = _poolKey(testToken, TSLA);
        bool zeroForOne = Currency.unwrap(key.currency0) == TSLA;
        inputs[2] = _encodeV4ExactInFromRouter(key, zeroForOne, 1);
        inputs[3] = abi.encode(TSLA, ethBuyer, uint256(0));
    }

    /// @dev Settle router-held TSLA, swap OPEN_DELTA, take TEST to msg.sender.
