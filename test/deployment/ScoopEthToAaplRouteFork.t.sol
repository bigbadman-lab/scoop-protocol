...
/// @notice Phase 6C.4 fork proof: post-launch ETH -> AAPL -> TOKEN in one Universal Router tx.
contract ScoopEthToAaplRouteForkTest is Test {
    ...
    function test_fork_launchTimeDevBuyRequiresAapl() public {
        _configureAaplOnFork();
        ScoopFactory.LaunchParams memory params = _params("NeedAapl", "NUG", bytes32(uint256(801)));
        uint256 fee = FACTORY.LAUNCH_FEE();
        vm.startPrank(creator);
        IERC20(AAPL).approve(address(FACTORY), 0.1e18);
        vm.expectRevert();
        FACTORY.launchAndBuy{value: fee}(params, 0.1e18, 1);
        vm.stopPrank();
        assertEq(IERC20(AAPL).balanceOf(creator), 0);
    }

    function test_fork_ethOnlyBuyerSingleTxBuysTest() public {
        _configureAaplOnFork();
        _launchTestAaplWithCreatorBuy();

        assertGt(ethBuyer.balance, 0);
        assertEq(IERC20(AAPL).balanceOf(ethBuyer), 0);
        assertEq(IERC20(testToken).balanceOf(ethBuyer), 0);

        uint256 ethIn = 0.02 ether;
        uint256 aaplQuoted = _quoteWethToAapl(ethIn);
        assertGt(aaplQuoted, 0, "ETH->AAPL quote failed");
        console2.log("quoted AAPL for 0.02 ETH", aaplQuoted);

        uint256 factoryEthBefore = address(FACTORY).balance;
        uint256 factoryAaplBefore = IERC20(AAPL).balanceOf(address(FACTORY));
        ...
        IUniversalRouter(UNIVERSAL_ROUTER).execute{value: ethIn}(
            ...
            _routeInputs(ethIn, aaplQuoted),
            block.timestamp + 60
        );

        uint256 testBal = IERC20(testToken).balanceOf(ethBuyer);
        uint256 aaplBal = IERC20(AAPL).balanceOf(ethBuyer);
        uint256 wethBal = IERC20(WETH).balanceOf(ethBuyer);
        console2.log("buyer TEST", testBal);
        console2.log("buyer AAPL residual", aaplBal);
        console2.log("buyer WETH residual", wethBal);

        assertGt(testBal, 0, "buyer received no TEST");
        assertLe(aaplBal, 1e12, "unexpected AAPL residual");
        assertEq(wethBal, 0, "unexpected WETH residual");

        ScoopFactory.Launch memory rec = FACTORY.getLaunch(testToken);
        assertEq(rec.quoteAsset, AAPL);
        assertEq(PoolId.unwrap(rec.poolId), PoolId.unwrap(testPoolId));
        assertEq(address(FACTORY).balance, factoryEthBefore);
        assertEq(IERC20(AAPL).balanceOf(address(FACTORY)), factoryAaplBefore);
        assertEq(IERC20(testToken).balanceOf(address(FACTORY)), 0);
    }

    function _configureAaplOnFork() internal {
        vm.startPrank(AUTHORITY);
        if (!PRICE_ORACLE.isConfigured(AAPL)) {
            PRICE_ORACLE.configureFeed(AAPL, AAPL_USD_FEED, STOCK_MAX_AGE);
        }
        if (!QUOTE_REGISTRY.isRegistered(AAPL)) {
            QUOTE_REGISTRY.registerQuote(AAPL, ScoopQuoteRegistry.QuoteType.Stock);
        }
        vm.stopPrank();
        assertTrue(QUOTE_REGISTRY.isEnabled(AAPL));
        assertTrue(PRICE_ORACLE.isEnabled(AAPL));
    }
    ...
    function _launchTestAaplWithCreatorBuy() internal {
        ...
        uint256 fee = FACTORY.LAUNCH_FEE();
        vm.startPrank(creator);
        IERC20(AAPL).approve(address(FACTORY), quoteIn);
    ...
    function _quoteWethToAapl(uint256 amountIn) internal returns (uint256 amountOut) {
        (amountOut,,,) = IQuoterV2(V3_QUOTER)
            .quoteExactInputSingle(
                IQuoterV2.QuoteExactInputSingleParams({
                    tokenIn: WETH, tokenOut: AAPL, amountIn: amountIn, fee: V3_FEE, sqrtPriceLimitX96: 0
                })
            );
    }

    function _routeInputs(uint256 ethIn, uint256 quotedAapl) internal view returns (bytes[] memory inputs) {
        inputs = new bytes[](4);
        ...
        uint256 minAapl = quotedAapl * 99 / 100;
        bytes memory v3Path = abi.encodePacked(WETH, V3_FEE, AAPL);
        inputs[1] = abi.encode(ActionConstants.ADDRESS_THIS, ActionConstants.CONTRACT_BALANCE, minAapl, v3Path, false);

        PoolKey memory key = _poolKey(testToken, AAPL);
        bool zeroForOne = Currency.unwrap(key.currency0) == AAPL;
        inputs[2] = _encodeV4ExactInFromRouter(key, zeroForOne, 1);
        inputs[3] = abi.encode(AAPL, ethBuyer, uint256(0));
    }

    /// @dev Settle router-held AAPL, swap OPEN_DELTA, take TEST to msg.sender.
